...
int cesarEncoding(unsigned char* input, size_t len, void** extra) {
    int op_type = * (int*) extra[0];
    int offset = atoi((char*) extra[1]);
    for (size_t i = 0; i < len; ++i) {
        if (op_type == CODE_OP)
            input[i] = (input[i] + offset)%TOTAL_CARACTERES;
        else
            input[i] = (input[i] - offset)%TOTAL_CARACTERES;
    }
    return 0;
...
int vigenereEncoding(unsigned char* input, size_t len, void** extra) {
    int op_type = *(int*) extra[0];
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (op_type == CODE_OP)
            input[i] = (input[i] + key[pos_clave])%256;
        else
            input[i] = (input[i] - key[pos_clave])%256;
    }
    *current_pos += len;
...
int rc4Encoding(unsigned char* input, size_t len, void** extra) {
    unsigned char* arreglo_random = (unsigned char*) extra[0];
    size_t* pos_1 = (size_t*) extra[1];
    size_t* pos_2 = (size_t*) extra[2];

    for (size_t i = 0; i < len; ++i) {
        input[i] = (unsigned char) input[i] ^ rc4Output(arreglo_random,
                                                         pos_1, pos_2);
    }
